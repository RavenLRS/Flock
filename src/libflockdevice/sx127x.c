...
#define SX127X_FSK_FREQ_STEP 61.03515625f // 61khz
...
// Each DIO definition takes 2 bits
#define DIO_MASK(offset) (3 << (offset))
#define DIO_CLR_MASK(offset) (~DIO_MASK(offset))
...
// DIO5 is configured via REG_DIO_MAPPING_2
#define DIO5_BIT_OFFSET 4
#define DIO5_FSK_CLKOUT (0 << DIO5_BIT_OFFSET)
#define DIO5_FSK_MODE_READY (3 << DIO5_BIT_OFFSET)
#define DIO5_LORA_CLKOUT (2 << DIO5_BIT_OFFSET)
#define DIO5_LORA_MODE_READY (0 << DIO5_BIT_OFFSET)
...
#define REG_DIO_MAPPING2_CLK_OUT (1 << 5)
...
static uint8_t sx127x_read_reg(sx127x_t *sx127x, uint8_t addr)
{
    ...
    uint8_t dummy = 0;
    uint8_t out;
    flock_sx127x_spi_transmit(0, addr, &dummy, 1, &out, 1);
    return out;
}

static void sx127x_write_reg(sx127x_t *sx127x, uint8_t addr, uint8_t value)
{
    flock_sx127x_spi_transmit(1, addr, &value, 1, NULL, 0);
}
...
static void sx127x_reset(sx127x_t *sx127x)
{
    ...
    flock_sx127x_set_rst(0);
    flock_millis_delay(10);
    flock_sx127x_set_rst(1);
    flock_millis_delay(15);
}

bool sx127x_init(sx127x_t *sx127x)
{
    sx127x_reset(sx127x);
    ...
    {
        FLOG_E(TAG, "Unexpected SX127X chip version %u, expecting %d", version, SX127X_EXPECTED_VERSION);
        return false;
    }
    FLOG_I(TAG, "Got SX127X chip version %u", version);
...
void sx127x_set_frequency(sx127x_t *sx127x, unsigned long freq, int error)
{
    ...
    if (frf > 0)
    {
        ...
        // TODO: Wait up to 50us for PLL lock (page 15, table 7)
#if 0
...
void sx127x_set_op_mode(sx127x_t *sx127x, sx127x_op_mode_e op_mode)
{
    ...
    {
        // OpMode can only be changed while the radio is in SLEEP mode
        sx127x_sleep(sx127x);
        uint8_t prev_mode = sx127x->state.mode & SX127X_OP_MODE_MODE_MASK;
        switch (op_mode)
        {
        case SX127X_OP_MODE_FSK:
...
void sx127x_send(sx127x_t *sx127x, const void *buf, size_t size)
{
    ...
    // Write payload
    flock_sx127x_spi_transmit(1, REG_FIFO, ptr, ptr_size, NULL, 0);
...
size_t sx127x_read(sx127x_t *sx127x, void *buf, size_t size)
{
    ...
    flock_sx127x_spi_transmit(0, REG_FIFO, NULL, ptr_size, ptr, 0);
    sx127x->state.rx_done = false;
...
int sx127x_frequency_error(sx127x_t *sx127x)
{
...
#if 0
        flock_sx127x_spi_transmit(0, REG_FSK_FEI_MSB, NULL, 2, rx_data, 0);
    ...
    case SX127X_OP_MODE_LORA:
    {
        ...
        flock_sx127x_spi_transmit(0, REG_LORA_FEI_MSB, NULL, 3, rx_data, 0);
...
int sx127x_rssi(sx127x_t *sx127x, int *snr, int *lq)
{
    ...
    {
        rssi_max_dbm = 1;
        flock_sx127x_spi_transmit(0, REG_LORA_PKT_SNR_VALUE, NULL, sizeof(rx_data), rx_data, 0);
...
sx127x_clkout_div_e sx127x_clkout_div(sx127x_t *sx127x)
{
    // RegOsc is only available on FSK mode
    if (sx127x->state.op_mode != SX127X_OP_MODE_FSK)
    {
        return -1;
    }
    uint8_t reg_osc = sx127x_read_reg(sx127x, REG_FSK_OSC);
    // ClkOut config is in the LS 3 bits
    return reg_osc & 7;
}

bool sx127x_set_clkout_div(sx127x_t *sx127x, sx127x_clkout_div_e clkoutdiv)
{
    if (sx127x->state.op_mode != SX127X_OP_MODE_FSK)
    {
        return false;
    }
    if (clkoutdiv < SX127X_CLKOUT_DIV_1)
    {
        clkoutdiv = SX127X_CLKOUT_DIV_1;
    }
    if (clkoutdiv > SX127X_CLKOUT_DIV_OFF)
    {
        clkoutdiv = SX127X_CLKOUT_DIV_OFF;
    }
    // ClkOut config is in the LS 3 bits
    uint8_t reg_osc = sx127x_read_reg(sx127x, REG_FSK_OSC) & ~7;
    reg_osc |= clkoutdiv;
    sx127x_write_reg(sx127x, REG_FSK_OSC, reg_osc);
    return true;
}

bool sx127x_is_clkout_enabled(sx127x_t *sx127x)
{
    uint8_t dio5 = sx127x_read_reg(sx127x, REG_DIO_MAPPING_2) & DIO_MASK(DIO5_BIT_OFFSET);
    switch (sx127x->state.op_mode)
    {
    case SX127X_OP_MODE_FSK:
        // Table 30, only 00 enables CLKOUT
        return dio5 == (0 << DIO5_BIT_OFFSET);
    case SX127X_OP_MODE_LORA:
        // See table 18. Both 01 and 10 enable ClkOut, but 00 and 11 don't
        return dio5 == (1 << DIO5_BIT_OFFSET) || dio5 == (2 << DIO5_BIT_OFFSET);
    }
    return false;
}

void sx127x_set_clkout_enabled(sx127x_t *sx127x, bool enabled)
{
    uint8_t dio_mapping_2 = sx127x_read_reg(sx127x, REG_DIO_MAPPING_2) & DIO_CLR_MASK(DIO5_BIT_OFFSET);
    switch (sx127x->state.op_mode)
    {
    case SX127X_OP_MODE_FSK:
        // Table 30, only 00 enables CLKOUT
        if (enabled)
        {
            dio_mapping_2 |= DIO5_FSK_CLKOUT;
        }
        else
        {
            dio_mapping_2 |= DIO5_FSK_MODE_READY;
        }
        break;
    case SX127X_OP_MODE_LORA:
        if (enabled)
        {
            dio_mapping_2 |= DIO5_LORA_CLKOUT;
        }
        else
        {
            dio_mapping_2 |= DIO5_LORA_MODE_READY;
        }
        break;
    }

    sx127x_write_reg(sx127x, REG_DIO_MAPPING_2, dio_mapping_2);
}
...
static uint8_t sx127x_get_fsk_bandwidth_reg_value(unsigned hz)
{
    ...
    FLOG_E(TAG, "Invalid FSK bandwidth %u", hz);
    UNREACHABLE();
}
